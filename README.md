...
- Собрать список клиентов из Active Directory.
...
- `S` — обновить список клиентов из Active Directory.
...
`tracert` и `gpupdate /force` открываются в отдельном окне cmd, показывают результат и ждут нажатия клавиши. После этого окно можно закрыть без лишнего интерактивного режима.

## Синхронизация клиентов из Active Directory

Пункт `S` запускает `Sync-QuickRemoteToolkitClients.ps1`.

Скрипт работает без RSAT-модуля `ActiveDirectory`: он использует LDAP через встроенный .NET `DirectorySearcher`.

По умолчанию он:

- читает компьютеры из текущего домена;
- пропускает отключенные объекты;
- пропускает компьютеры, которые не входили в домен больше 180 дней;
- пытается получить IPv4 через DNS;
- заполняет поле “Имя сотрудника” из `description` объекта компьютера в AD;
- оставляет поле “Имя сотрудника” пустым, если `description` не заполнен;
- сохраняет результат в `QuickRemoteToolkit.clients.csv`.

Ручной запуск:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Sync-QuickRemoteToolkitClients.ps1
```

Полезные параметры:

```powershell
# искать только в конкретной OU
.\Sync-QuickRemoteToolkitClients.ps1 -SearchBase "OU=Workstations,DC=example,DC=local"

# не отбрасывать старые компьютеры
.\Sync-QuickRemoteToolkitClients.ps1 -InactiveDays 0

# включить отключенные объекты AD
.\Sync-QuickRemoteToolkitClients.ps1 -IncludeDisabled

# не резолвить IP через DNS
.\Sync-QuickRemoteToolkitClients.ps1 -NoDnsResolve
```
